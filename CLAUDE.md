...
point task read only the § file you need in `spec/` plus any canon artifact it links, never @-import the spec
...
Adversarial security / threat-model reviews — red-teaming a spec §, abuse-case analysis, attack-surface probing of any future code — are **delegated to Codex** (a direct `codex exec` with a self-contained prompt), not run by Claude in the first person.
...
- Routing rule, not a license to ignore security: a concern noticed in passing still gets surfaced plainly (issue / § edit) — the adversarial probing is what goes to Codex.
