...
def test_fresh_init_and_idempotent_reopen_are_private_and_versioned(
    ...
    assert first_status.supported_version == 12
...
def test_incompatible_schema_blocks_business_io_but_status_reports(
    ...
    with sqlite3.connect(database) as connection:
        connection.execute(
            ...
            (13, FIXED_NOW.isoformat(), "future-build"),
        )
    ...
    assert envelope["result"]["schema"]["stored_version"] == 13
...
def test_incompatible_workspace_blocks_file_capture_before_source_read(
    ...
    connection.execute(
        "INSERT INTO schema_meta(version, applied_at, app_version) VALUES (13, ?, ?)",
