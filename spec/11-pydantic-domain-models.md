...
54. Every accepted `datetime` must have a representable UTC instant under §16.7 rule 3, and a non-range `OccurredAt` must additionally have a representable §16.7 rule 6 interval — that UTC instant plus its rule 5 width. A value close enough to either end of the calendar to fail either test is refused here, not at whichever consumer normalizes it first; refusal is owner-visible input failure (§14.14), never an integrity fault.
