...
V1 reserves, but does not implement, a future human-only `private` marker as either a field or named metadata key; when introduced, it must bind at the §15 input-assembly boundary and exclude the marked `RawLog` from every LLM-stage input, including an otherwise user-initiated run. Until then the V1 answer for material the owner will not disclose is provider selection, not a per-record marker: choosing a provider chooses who may see every §29.3 data class (§29.2), and §15.13's required `openai-compat` adapter makes a self-hosted or on-device OpenAI-compatible endpoint a configuration-only switch. Against a shared provider, declining to capture the record remains V1's only way to withhold one record while retaining the rest.
