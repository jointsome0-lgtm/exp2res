...
Canonical serialization uses UTF-8 JSON and declared field names only. For the §12.15 `input_hash` and `output_hash`, keys are sorted and insignificant whitespace is omitted; two conforming implementations therefore hash identical validated inputs and outputs identically.
