...
11. Every top-level §11 entity `id` derives to `TEXT PRIMARY KEY`, the unique parent key used by rule 6 foreign keys and rule 10 resolution.
    ...
    - That collision-resistant random allocation is itself the whole anti-reuse mechanism. A purged identifier is unreachable because no allocation can plausibly produce it again, not because anything remembers it: the workspace retains no ledger, tombstone, or other record of purged IDs, and no allocator consults one. Retaining the identifiers a §13.13 purge just removed would contradict §5's owner deletion authority and leave the deletion times of purged records readable, so any such record is non-conforming.
...
14. `raw_logs` and `experience_facts` — the tables storing copied project provenance — additionally derive one service-set storage column:
    ...
    - Every project-identity comparison consumes the stored key rather than re-implementing normalization over labels. V1 defines no such comparison consumer: the project-scoped assessment view is not a V1 view (§13.6), so the key is stored comparison identity awaiting its first consumer.
...
The project-scope removal (2026-08-07, A4) names its deterministic path the same way, because dropping the `AssessmentScope` `project` member and the `scope_target` field would otherwise leave retained project-scoped rows without a legal transform: that migration deletes, inside its transaction, every retained `assessment_snapshots` row whose stored scope is `project` — current and superseded — together with everything owned by or anchored to it: its self-claims, its verification findings, and every resume branch, resume bullet, and finding reached through those snapshots. Global-scoped rows are retained and keep every value except the dropped `scope_target`, whose removal is a column drop, not a content change. Raw logs, evidence items, experience facts, detection sets, job descriptions, and run telemetry are untouched, and no global-scoped derivation is regenerated. Managed-output cleanup follows the signal-layer mechanics exactly, and covers retained rows too: every published set predates this migration and therefore carries the superseded `manifest_version = 5` and companion `schema_version = 2`, which §13.14 rule 5 refuses to overwrite in place. The migration therefore attempts removal of every `out/assessment/<snapshot-id>/` and `out/branch/<branch-id>/` set it can reach — deleted and retained alike — through §13.14 before commit, reporting any residual path as an unsuccessful invalidation, so the retained global view re-exports cleanly at version 6 instead of stalling on an owner-removable residual. This whole-layer deletion is that migration's defined deterministic transform, not a silent drop or default.
