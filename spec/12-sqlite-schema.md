...
11. Every top-level §11 entity `id` derives to `TEXT PRIMARY KEY`, the unique parent key used by rule 6 foreign keys and rule 10 resolution.
    ...
    - That collision-resistant random allocation is itself the whole anti-reuse mechanism: a purged identifier is unreachable because no allocation can plausibly produce it again, not because anything remembers it. No ledger, tombstone, or other state retained for anti-reuse records purged IDs, and no allocator consults one; §12.13's retained `input_ids_json`/`output_ids_json` telemetry is not such state.
