...
Like every §16 rule, this one binds verification runs, not stored history: a persisted verdict is the immutable record of the rule set its Stage 7 run applied, and a pre-adoption verdict keeps that recorded status until the owner's next §13.7 re-verification — which always applies the current rules and supersedes dependent state through §13.7's own invalidation. No upgrade gate retroactively invalidates derived state over phrasing form.
