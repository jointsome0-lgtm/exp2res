...
`displaced_support_items` contains exactly one descriptor for each non-`manual_claim` `EvidenceItem` linked to a displaced record of the lineage, ID-ordered ascending by byte order. Each descriptor has exactly `id`, `raw_log_id`, `strength`, `title`, `uri`, and `path`; it omits `summary`, `created_at`, `metadata`, every `RawLog` field, the record's `raw_text`, and any copied gap-question context. It is a call-time projection of retained rows, not a persisted resolution artifact. A displaced `manual_claim` item never appears in either evidence array. The example array is empty because its lineage has no correction.

An extractor may select a displaced-support descriptor's `id`, and then includes its `raw_log_id` in `source_log_ids`, only as support within that item's §9.4 evidential scope. The descriptor is never a content source: every emitted fact and every content-bearing field must trace to content supplied by the effective `raw_logs` and `evidence_items`. Thus a correction may restate a corrected imported fact and select both the correction's item and the displaced root's non-manual descriptor, but no displaced raw text, summary, copied question context, or manual claim can be re-emitted.
...
Every fact output copies `project` exactly from the governing record under §13.3 rules 10 and 13 — `None` when that record carries none; an authored, renamed, or re-cased project value is invalid structured output, because §13.6 project views select subjects through this field.

Every fact output also carries `occurred`. It copies the governing record's `OccurredAt` under §13.3 rule 10 by default; when the root is the only effective record, the root governs. It may emit a contained narrower placement only when the selected effective-record context explicitly states the narrower time; this is the additional linked support required by §16.7, not a model inference. It may never widen beyond the governing record's source window, set `occurred.precision` / persisted `temporal_precision` stronger than the strongest explicit in-context temporal support, or set `occurred.confidence` / persisted `temporal_confidence` above the governing record's temporal confidence under §10's order. When support conflicts or containment cannot be established, preserve the governing placement and lower temporal confidence if necessary rather than change its window or choose a stronger one.
