...
Stage 10 invokes this contract exactly once per pack under §13.10's input-selection rules, and `bullets` is the complete typed bullet array for that pack. It is non-empty: an empty array is invalid structured output, because §15.7 requires a non-empty `resume_bullets` and an empty branch would pass every export check vacuously. A run whose supplied context supports no bullet completes as a §14.14 class-10 `blocked` result — no branch, no bullet, and no partial commit — rather than committing an empty branch. Each supplied claim arrives as its complete §11.6 object, so its `counter_fact_ids` contrary-role marking stays visible: a fact marked counter on a claim grounds no bullet wording through that claim, and Stage 11 judges against the same marking. The contract does not pass `AssessmentSnapshot.title` or `.summary` as independent prose inputs.
