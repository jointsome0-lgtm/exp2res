...
Example notation: each entity's full-shape example lives once, at its producing contract — §15.2 for `ExperienceFact`, `EvidenceItem`, and `RawLog`; §15.4 for `SelfSignal` and `GapQuestion` inputs and `SelfClaim` output; §15.6 for a verified `SelfClaim` input; §15.9 for `ParsedJD`. Other examples elide a repeated body to a `"<id: complete §NN.N Model — canonical example in §NN.N>"` string. The placeholder is example notation only: the service always passes the complete typed objects the surrounding prose requires.
