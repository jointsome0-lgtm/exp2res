...
9. **Failure, cancellation, and telemetry mapping.** §15.10 governs unchanged. A deadline or timeout kill is `transport_timeout`; a missing or unreadable final-message file or a nonzero runtime exit is `transport_provider_error` unless the adapter's declared error channel deterministically classifies `transport_auth_failed` or `transport_rate_limited`; cancellation kills the process group, leaves no partial generation, and records `cancelled`. One planned invocation remains one §12.15 call row. Runner identity — adapter identifier, runtime version, sandbox mechanism, and protocol version — is privacy-safe execution metadata within §12.13's existing columns and `metadata_json` under the same content prohibition; the protocol introduces no schema change and no new telemetry channel.
