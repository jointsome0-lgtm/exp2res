...
Reference implementations are declaration-gated, never version-pinned. The Codex CLI adapter is the version 1 reference: native schema-constrained final output with a dedicated final-message file (`--output-schema`, `--output-last-message`), ephemeral and ambient-configuration-disabled invocation (`--ephemeral`, `--ignore-user-config`, and `--ignore-rules` for user/project execpolicy rules files — each ambient layer the CLI declares a disable control for is disabled, per rule 3), explicit working directory (`-C`), with its read-only sandbox mode retained only as defense in depth under rule 2. The secondary Claude Agent SDK headless adapter must satisfy the identical rules through its declared controls — disallowed tools, isolated working directory, no user settings or instruction-file ingestion, native structured final output — and produce the same rule 8 canary evidence. A runtime that cannot execute as a confinable child process cannot declare conformance to this protocol.
