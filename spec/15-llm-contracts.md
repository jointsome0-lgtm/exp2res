...
Reference implementations are declaration-gated, never version-pinned. The Codex CLI adapter is the version 1 reference: native schema-constrained final output with a dedicated final-message file (`--output-schema`, `--output-last-message`), ephemeral and ambient-configuration-disabled invocation (`--ephemeral`, `--ignore-user-config`, and `--ignore-rules` for user/project execpolicy rules files — each ambient layer the CLI declares a disable control for is disabled, per rule 3), an explicitly pinned non-interactive approval policy (`-c approval_policy="never"`; the tested `exec` surface exposes no dedicated approval flag, and rule 3 forbids leaving a tool-policy parameter to the runtime default), explicit working directory (`-C`), with its read-only sandbox mode retained only as defense in depth under rule 2. The secondary Claude Agent SDK headless adapter must satisfy the identical rules through its declared controls — disallowed tools, isolated working directory, no user settings or instruction-file ingestion, native structured final output — and produce the same rule 8 canary evidence. A runtime that cannot execute as a confinable child process cannot declare conformance to this protocol.
...
1. **`codex-cli` (default).** The agent-backed Codex CLI runner — §15.12's version 1 reference — with default model `gpt-5.6-sol` and an externally-managed session (§29.2). §14.1 writes this selection into a fresh `config.toml`; it transmits nothing until the owner establishes the external CLI session, and a missing or unauthenticated session fails the outward call closed.
2. **`claude-agent-sdk` (secondary).** The agent-backed Claude Agent SDK headless adapter with default model `claude-opus-4-8`, an externally-managed session, and the identical §15.12 rules and §21.50 isolation evidence. It is the configured switch target when the owner judges the default model's §16 verifier-gate quality insufficient.
3. **`openai-compat` (required direct transport).** The one non-agent-backed adapter: a thin OpenAI-compatible HTTP transport with a configurable `base_url`, model, §29.2 credential reference, and a per-model §15.10 rule 4 declaration covering context/output bounds, structured-output mode — strict JSON-Schema or JSON-object, with §15.1's single validation retry absorbing the difference — and optional pricing. It serves users without either subscription, is the only V1 adapter exercising §29's credential references, and owns its transport directly: no multi-provider abstraction layer may sit between it and §15.10's stable failure-code mapping, deterministic preflight, and echo-free contracts. A native Anthropic Messages API adapter is explicitly outside V1.
