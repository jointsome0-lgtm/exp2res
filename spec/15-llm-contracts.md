...
9. **Budgets and privacy-safe progress/cost reporting.** Budget values live in `.exp2res/config.toml` under `[llm]`: per-request-round transport-attempt cap, lower and upper backoff bounds, per-invocation deadline, input and output token budgets, per-run call ceiling, and applicable per-invocation and per-run cost ceilings. The service supplies conservative defaults. These numeric values are configuration, so tuning them is not an SDD change; the existence of each budget, deterministic estimation where applicable, fail-before-call enforcement, and the shipped-default relationship below are normative.
   ...
   The documented expected upper bound for the default §15.13 selection at its default effort is 130 seconds for one provider invocation, and the shipped `invocation_deadline_seconds` default must be at least that bound in both the fresh-workspace template and the built-in budget configuration. Basis: during the 2026-07-25 first live-provider run, one self-signal extraction over an 18-fact corpus on `codex-cli` / `gpt-5.6-sol` at `high` measured roughly 130 seconds; `600.0` seconds was the deadline that made the identical recompute succeed on its first attempt. This measurement establishes the default-sizing floor, not a latency guarantee.
   ...
   A failed invocation names its surface on the same channel and under the same prohibition: human mode prints one diagnostic naming the failing stage and contract identifier alongside rule 10's stable failure code, so the owner learns which surface broke without reading telemetry rows. Both values are service-owned constants, so the line carries no provider bytes, no payload content, and no credential; the provider's own error channel is never echoed, quoted, summarized, or persisted. The obligation follows the run, not the call: an operational failure the stage orchestrator raises after a call has already validated — deterministic enrichment, the complete-stage commit — names the same stage and contract, because the owner's question is which surface broke rather than where in the service it was detected. This is a human-mode diagnostic only — the §14.14 result envelope is unchanged, and the same run remains inspectable through §14.13 by the `run_ids` the envelope already reports.
10. **Stable failure codes.** `processing_runs.failure_code` and `llm_calls.failure_code` use stable machine codes and are intentionally not §10 enums (§12.13). The minimum transport/runtime vocabulary is `transport_timeout`, `transport_rate_limited`, `transport_provider_error`, `transport_request_rejected`, `transport_model_unavailable`, `transport_lost_response`, `transport_auth_failed`, `capability_mismatch`, `budget_exceeded`, `context_overflow`, and `cancelled`; §15.1 validation and reference failures retain their separately governed stable codes. Response or HTTP timeouts and per-invocation deadline exhaustion use `transport_timeout`; HTTP 429 uses `transport_rate_limited`; ambiguous delivery uses `transport_lost_response`; authentication or authorization failure uses `transport_auth_failed`; connection/TLS failure, provider overload, and HTTP 5xx use `transport_provider_error`. The vocabulary is append-only, and these codes are outcome classes rather than provider messages: each names a distinct owner remedy, which is the only reason to separate one from the catch-all. A provider that rejects the request itself — a refused output schema, a malformed or unacceptable request shape, an unsupported parameter — uses `transport_request_rejected`, because the remedy is a build or configuration fix rather than waiting out a provider fault; a provider that does not recognize or will not serve the selected model uses `transport_model_unavailable`, because the remedy is the `[llm]` selection. Both are non-retryable and stay in the provider/transport exit class, and every remaining non-retryable HTTP 4xx keeps `transport_provider_error` — an adapter that cannot classify a rejection shape by its own deterministic markers leaves it in that catch-all rather than guessing a narrower code. Because rule 9 acknowledges that a provider's error channel may carry source-derived text, nothing found in that channel says who wrote it — not a phrase, not an exact error-code token, not a well-formed error body, since owner text can contain each verbatim: a rejection class is named only from a typed field the adapter parsed out of its own runtime's envelope, which for a CLI adapter means the status it reports there, and an adapter whose runtime reports no such field names no rejection class at all rather than reading one out of a process channel — it keeps `transport_provider_error`, a less specific remedy but never a wrong one, which is what the catch-all is for; and classification runs only after that adapter's retryable-outage markers, so a channel that names an outage keeps its retry even when it also echoes rejection wording. Losing a legitimate retry is the worse error, and both new classes name a remedy no retry could reach anyway. A recovered retry may finish successfully with a nonzero retry counter and no terminal failure code. From the first runner implementation, each failed attempt increments the applicable §12.15 retry counter before another attempt, and each terminal failure records its stable class on the same logical call row; transport/schema validity is never left as an unclassified SDK event.
