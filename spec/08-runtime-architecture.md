...
37. The writer lock establishes the identity of what it covers: the database, read beside the lock entry itself rather than through the workspace pathname a second time, and the managed-output roots the lock finds in place. §13.14 rule 9 binds every managed-output mutation to that identity for as long as the lock is held.
