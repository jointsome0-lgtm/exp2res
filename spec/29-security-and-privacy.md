...
The isolated-runner child can see only the declared typed input channel: its OS-sandbox wrapper binds the fresh per-call workspace, auth material, and TLS/DNS essentials, but no repository, source tree, workspace database, user home, or user Codex rules. The CLI separately supplies rules and ambient-configuration isolation. Read confinement is wrapper-provided and its source of truth is the §21.50 effectiveness canary, not any prose enumeration of host binds; an unavailable or ineffective wrapper fails the two-half capability preflight closed before transport.
