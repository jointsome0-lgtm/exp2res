...
The Gaps section renders status-bearing snapshot claims with `dimension = "gap"`. The Unknowns section renders no free-form snapshot prose: for every ID in the snapshot's complete current unanswered `gap_question_ids` set, it presents the referenced `GapQuestion.target_*`, `reason`, and `priority` as missing-information context. Questions Worth Answering renders those same rows' `question` values. Missing, duplicate, superseded, answered, omitted, or writer-output-inconsistent IDs fail before rendering under §12 rule 10 and the Stage 6 complete-state check. These references present uncertainty; they do not receive a status, alter §16.11 aggregation, or become independent inputs to Stage 10. Any declarative conclusion must instead be a status-visible `SelfClaim`.
