...
The bound URLs are progress output on standard error in both modes and never on stdout, so `--json` stdout still carries exactly one envelope. Serving ends on user interruption under §14.14 rule 6 with exit class 9, `status = cancelled`, and `result = null`: each served request is already complete when its response is written, and the command itself reaches no completed primary result. Interruption stops accepting connections first, then drains: a request already accepted runs to its own §30 rule 7 outcome and writes that response before the envelope is emitted, so no answered request ends without the outcome that rule promises. Draining is bounded rather than open-ended, and by socket deadlines rather than by the database alone: §30 rule 6 already confines each read transaction to the server's own resolution work, and every accepted connection additionally carries a receive deadline for its complete request and an emit deadline for its response, both fixed at §8.1's bounded contention timeout so serving adds no new tunable. Neither expiry weakens §30 rule 7. A receive deadline that expires produced no complete request for the server to answer, and an emit deadline that expires stops delivery to a client that stopped reading, after that request already reached its outcome — the guarantee is that every request the server answers gets exactly one outcome, never that a client stays connected to receive it. A second interruption abandons the drain and closes the remaining connections unanswered; that is the owner overriding rule 7's guarantee, not the server breaking it, and the envelope stays exit class 9 either way. Individual requests never appear in the envelope, and §30 rule 6 bars request bytes from every diagnostic line, so progress output names the route and outcome class only.
