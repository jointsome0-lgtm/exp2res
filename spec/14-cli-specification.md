...
The bound URLs are progress output on standard error in both modes and never on stdout, so `--json` stdout still carries exactly one envelope. Serving ends on user interruption under §14.14 rule 6 with exit class 9, `status = cancelled`, and `result = null`: the command itself reaches no completed primary result. During ordinary serving every admitted connection carries a receive deadline for its complete request and an emit deadline for its complete response, both fixed at §8.1's bounded contention timeout so serving adds no new tunable. The receive deadline is one absolute deadline that begins when §30 rule 10 acquires the admission slot immediately after `accept`; receiving another byte never pauses, restarts, or replaces it. Receive expiry closes a connection that produced no complete request, emits no HTTP response, and releases its slot. The emit deadline begins before the first response byte: the server has already composed the request's one §30 rule 7 outcome and closed its read transaction, but expiry closes the connection without retry and may leave the peer with no response bytes or only a partial response. That transport failure neither changes the computed outcome nor invents another response, and §30's completed-response guarantee does not claim successful delivery to a client that stopped reading.
