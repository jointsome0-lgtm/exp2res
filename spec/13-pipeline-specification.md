...
9. **Locked-database binding.** Every managed-set removal is bound to the identity of the database its caller holds under the §8.1 writer lock: the caller reads that identity while holding the lock and before any removal, and a removal proceeds only while the workspace pathname still resolves to it.
    ...
    - The identity is read through the same no-follow walk the comparison uses, so a substituted `.exp2res` cannot answer for a database the surrounding `out/` no longer belongs to.
    - A mismatch, or an identity that could not be read at all, is never permission to remove: every in-scope set is reported as an unsuccessful invalidation under §13.13 rule 6, exactly as a failed removal is, and the operation's own database mutation stays committed.
    - This binds removal only. The residual window §13.13 rule 6 already accepts is unchanged, because no filesystem offers an open SQLite connection's inode as a path.
