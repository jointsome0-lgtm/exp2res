...
9. **Locked-workspace binding.** Every managed-output mutation is bound to the identity §8.1 rule 37 establishes with the writer lock: it proceeds only while the workspace pathname still resolves to that database and to those managed-output roots. The lock alone cannot supply this — it pins the entry it was opened through, while every path in this § is re-resolved by name — so a workspace renamed and replaced under a held lock would otherwise leave a command committing to one tree and mutating another.
    - A mismatch, or an identity that could not be established at all, is never permission to mutate. A removal reports every in-scope set as an unsuccessful invalidation under §13.13 rule 6, exactly as a failed removal does, and the operation's own database mutation stays committed; a publication fails rather than completing, and whatever it had already written is reported the same way. The report names every set the operation stranded, unfiltered by what the pathname currently holds — under a mismatch that is another workspace's content, which is no evidence about this one.
    ...
    - An effect counts as invalidated only where it is both durable and proven under this binding, and is reported residual otherwise. Neither absence at the pathname nor a completed flush is such a proof once the pathname has changed hands.
    - A managed-output root, and the migration-backup store this binding covers on the same terms, is covered from the lock where the lock found it, and from the holding command's own creation of it otherwise. Nothing else establishes one: a root substituted beside an untouched database is a mismatch like any other, and so is a root standing where the lock found none and this command created none — adopting it would extend the binding to an entry no lock covers.
    - This binds managed-output mutation only. The residual window §13.13 rule 6 already accepts is unchanged, because no filesystem offers an open SQLite connection's inode as a path, and every mutation here resolves by name.
