...
9. **Locked-database binding.** Every managed-output mutation is bound to the identity §8.1 rule 30 establishes with the writer lock: it proceeds only while the workspace pathname still resolves to that database. The lock alone cannot supply this — it pins the entry it was opened through, while every path in this § is re-resolved by name — so a workspace renamed and replaced under a held lock would otherwise leave a command committing to one tree and mutating another.
