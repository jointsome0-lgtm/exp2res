...
6. A manual daily log, retrospective log, gap answer, or correction receives its linked `EvidenceItem(strength=manual_claim)` when the `RawLog` is persisted; there is no later normalization stage. That item is created and reported first. Each locator accepted through §14's repeatable owner-capture option creates one additional linked item with `strength="artifact_reference"`, `title=None`, `summary="Owner-supplied artifact reference."`, and `metadata={}`. A POSIX path or `file:` URI resolving to one is stored only as its symlink-resolved canonical real path in `path`; any other absolute URI scheme is stored byte-for-byte in `uri`; the unused locator field is `None`. Those items are created and reported in one canonical order computed from stored values alone — local `path` items before remote `uri` items, each group ascending by the stored value's UTF-8 bytes — so input order carries no meaning, and no storage insertion artifact participates in the order a §12.14 migration or §14.16 rebuild could disturb. These items share rule 5's all-or-none transaction with the raw record. §14 exclusively owns count and duplicate-input usage, §29.4 exclusively owns locator authorization and inertness, and §9.4 owns the unchanged calibration consequence.
...
13. `ExperienceFact.project` is copied provenance, exactly like the default `occurred` placement: Stage 3 sets it to its governing record's `project` value under rule 10 — including `None` — after the §15.2 response validates, and copies the governing record's stored `project_key` in the same assignment (§12 rule 14); the extractor does not emit either field (§15.11). §13.6 selection compares only the stored key against the case-folded canonical selector.
14. When a lineage's §15.2 call supplied a non-empty `displaced_support_items` array and no fact of the committed replacement generation selects any of its descriptors, the completed run's §14.14 result carries one deterministic service-owned warning of type `displaced_support_unselected` for that lineage, naming the lineage root and the count of unselected descriptors in its message — a bounded form constructible within §11's string limits at any legal descriptor count, because the warning is assembled after the replacement generation committed. The type is service-reserved: a model-authored warning carrying it in a validated §15.2 response is discarded before the result is assembled, so the result carries the service computation exactly once per affected lineage. The warning is presentation, not a gate: it persists nowhere, blocks nothing, and recurs on every such run, because legitimate omission exists — displaced support can be off-topic for every replacement fact — while §15.2's required-selection rule means a silent omission would otherwise leave §9.4's descriptor-borne ceilings unreachable without any owner-visible trace.
...
Each successful run validates one complete candidate for both output sets. Every candidate and persisted detection has a deterministic service-computed **structural key**: for a gap, the tuple (`target_type`, `target_id`, `reason`, `priority`); for a contradiction, its two `(ref_type, ref_id)` references as an unordered pair, because the detector has no canonical side ordering and a swapped left/right rerun is the same conflict. Detector-authored prose — `question`, `title`, `description` — is payload, never identity: keys derive only from typed validated fields, and no LLM or semantic-equivalence judgment participates in lifecycle identity. Each validated output set's key set is duplicate-free; a candidate containing two gaps or two contradictions with equal structural keys is invalid structured output under §15.1 and §15.8. Adding a key discriminator — to separate genuinely different simultaneous detections on one target or reference pair — requires a concrete demonstrated case and a Decision Log entry first.

Retention is decided per output set within the one validated candidate. The contradiction set is retained if and only if the candidate's contradiction structural-key set equals the current generation's. The gap set is retained if and only if the candidate's gap structural-key set equals the current generation's and every current gap has `answered = false`. A retained set keeps its existing rows, IDs, and prose — a paraphrased candidate with equal keys changes nothing — and its discarded candidate half persists nowhere else. A set that fails its condition is replaced completely, and exactly that set's prior rows become superseded in the same transaction; replacing one set never supersedes or rewrites the other set's retained rows, and inputs or output references to superseded rows are invalid. `answered` and `answer_log_id` are service lifecycle state, not detector content, and never enter the comparison. A current gap set containing any gap with `answered = true` is always replaced even when the key sets are equal: the detector derived its candidate from inputs that include the stored answer evidence, so a re-emitted gap is genuinely still open (§14.7); each replacement gap starts with `answered = false` and `answer_log_id = None`, and no question-to-answer link is re-created. When both sets are retained, Stage 4 supersedes nothing and the run records only privacy-safe telemetry; a direct §14.7 invocation then invalidates no upper layer or managed export, while inside the §14.12 flow the downstream stages still follow §13.13. One Stage 4 run creates one §13.4 `processing_runs` row whether it retains or replaces either set.

**Idempotent-rerun short-circuit.** Before planning its provider call, Stage 4 deterministically compares the invocation it would make against the most recent completed §13.4 run that recorded a first-call `input_hash` (§12.15). When that run's `provider`, `model`, and `prompt_policy_hash` (§12.13) equal the current selection's, its recorded `input_hash` equals the §11 canonical hash of this invocation's complete serialized input, and every current gap has `answered = false`, the run completes as a full retention of both current sets with no provider call and no `llm_calls` row. The equality is decisive because Stage 4 input state never reverts byte-identically — entity IDs are never reused (§12 rule 11) and capture only appends records — while every §13.13 privacy operation redacts every retained content hash or purges the compared state, so a surviving matching hash proves the current detection generation was produced from, or already retained against, exactly this input. A redacted hash, an absent prior run, a changed adapter, model, or contract revision, or any input difference disqualifies the short-circuit and the ordinary complete call proceeds. No V1 flag forces a provider call over an unchanged input, contract, and selection: with nothing new to interpret, a repeated call could change the current sets only by sampling variance, which is interpretation churn rather than evidence (§13.13 rule 3). A short-circuited run reports the same retained result as a candidate-comparison retention.
...
Stage 7 verdicts never retire, supersede, resolve, or annotate a Stage 4 detection. Verifier counterevidence and a claim's `contradicted` status judge claims, not detections: polarity and ownership between a claim-level verdict and a detection-level conflict are not computable from their typed references — a counterevidence entry and a contradiction may share a referenced fact while describing different conflicts — so no automated adjudication maps one onto the other. When verifier evidence refutes what a detection describes, the remedy is evidence-driven: capture or correct records under §14, re-extract, and let the next Stage 4 run regenerate the detection set from current inputs, omitting the conflict when those inputs no longer support it. §17 handles the coexistence window in presentation: every rendered contradiction row carries a fixed origin label naming it unadjudicated detector output, so a detection is never read as a verified conclusion beside the claim verdicts it coexists with.

Downstream invalidation follows the dependency graph of the replaced set, not the run. A replaced contradiction set atomically supersedes every current signal, claim, snapshot, resume branch, and resume bullet before those rows can be reused, because the current contradiction set is §13.5/§15.3 input. A replacement confined to the gap set supersedes every current claim, snapshot, resume branch, and resume bullet — the unanswered gap set is §13.6/§15.4 input and every snapshot references it — but never the current signal generation: gaps are no Stage 5 input, so superseding signals over a gap-only change would price a provider-backed Stage 5 regeneration whose inputs are byte-identical. Regenerating the invalidated layers requires their §14 triggers; the shared §14.12 flow regenerates Stage 5, while assessment views and resume branches require §14.9/§14.10 (§13.13).

The V1 Stage 4 producer may persist only gaps and contradictions whose polymorphic targets are effective-lineage Stage 1 evidence present in this input or current Stage 3 facts. Effective evidence that produced no fact remains visible and may receive a gap target; a displaced record or any item linked to it can be neither input nor target. Stage 4 rejects targets owned by Stage 5 or later; `DetectionRefType` (§10) closes the target domain to Stage 1 evidence and Stage 3 facts.
...
Stage 6 also derives `AssessmentSnapshot.title` deterministically from the snapshot's own persisted identity fields: the renderer-owned literal `Self-Assessment — Global` for `global`, and `Self-Assessment — <scope_target>` for `project`, where `<scope_target>` is that snapshot's persisted canonical pre-fold `scope_target`. The title is structural view metadata, never writer prose: no model output, owner flag, or configuration participates, and equal persisted (`scope`, `scope_target`) pairs derive equal titles. Like `scope_target` itself, the title carries the generating run's canonical pre-fold selector — a regeneration that re-cases the folded-equal view's selector re-derives both together — so within any snapshot the two fields always agree and §13.14's `snapshot_title` manifest identity stays deterministic per snapshot (§11.7, §15.11).
...
**Deterministic claim repair.** `assess repair` (§14.9) is Stage 6's second, non-LLM form: it regenerates one current assessment view by adopting persisted §11.14 `suggested_rewrite` values, with no provider call, no §15 contract, and no new model-authored text beyond what verifier finding history already holds. Preconditions are checked fail-closed before any mutation: the selected snapshot is current (`selector_not_found` / `snapshot_not_current` otherwise); no member claim is `unverified` (`snapshot_not_verified`); at least one member claim's status is `rejected` or `unsupported` (`nothing_to_repair`); and each such repairable claim's latest finding — its §11.14 row with the greatest (`created_at`, ID) — carries a non-null `suggested_rewrite` (`rewrite_unavailable`). A violated precondition fails the command with its stable exit-class-2 diagnostic and changes nothing.

Repair constructs a new snapshot generation for the same view under the ordinary Stage 6 swap and §12 Stage 6 transaction checks. Every member claim is copied as a new row with a new ID, the new snapshot's ID, `verification_status = "unverified"`, and empty `counterevidence`; a repairable claim takes its adopted `suggested_rewrite` as `claim` while every other copied field is preserved, and every other claim copies its `claim` unchanged. `gap_question_ids` and `contradiction_ids` are recomputed fresh from current rows exactly like a generated snapshot; `summary` is service-copied from the possibly repaired `narrative_summary` member. The prior snapshot and its claims are superseded in the same swap, with the same stale-export and dependent branch/bullet consequences as a generated replacement.

Adoption is §11.14's one narrow carve-out: the persisted rewrite becomes a new unverified replacement-claim candidate. It is still never a prompt input and never export content, and it earns status only through an ordinary full Stage 7 re-verification of the new snapshot — repair pre-authorizes nothing, and that re-verification judges every member afresh, so a previously passing member's status may change; that is accepted, not a defect. The repair run is a non-LLM `processing_runs` row with stage `13.6` and NULL execution identity, owning zero §12.15 rows (§12.13, §12.15). The new snapshot's metadata carries `repaired_from_snapshot_id` naming the superseded snapshot, and each repaired claim's metadata carries `adopted_rewrite_of_claim_id` naming the prior claim whose latest finding supplied its text; both keys are inert provenance (§11).
...
13. On every non-summary claim, the `dimension` names what the claim characterizes under §15.4's categorization; a mis-categorizing dimension — an orientation dimension on a capability assertion, or the reverse — is `rejected`, because §17 keys section placement on this field and qualification cannot fix a routing value. The unique `narrative_summary` synthesizes across categories and §17 routes it by kind alone, so its dimension is not judged. Like §16.14, this check binds verification runs from adoption forward, never stored history: a pre-adoption verdict keeps its recorded status until the next re-verification, which applies current rules.
...
6. **Canonical containment and no-follow operations.** The writer requires the workspace's `out/` entry itself to be a real directory rather than a symlink, establishes its canonical real path, requires it to remain beneath the canonical workspace root, requires every existing descendant ancestor to be a real directory beneath that `out/` root, inspects each entry without dereferencing it, and performs destination creation relative to already validated parent directories. Every managed write, rename, manifest/member read, enumeration, and delete rechecks canonical root containment and uses no-follow semantics for every path component and final entry, extending §13.13 rule 6 from deletion to the complete managed-output surface. A symlink, a non-directory ancestor, an entry resolving outside the root, or a changed path between validation and operation is skipped and never traversed; every stable containment failure and every writer, deletion, or cleanup race is reported as residual, and this is true even when a symlink target is inside the workspace. One narrow reporting exception applies only to a §30 read: when a previously validated final entry changes before the reader's next no-follow operation, the reader skips that operation and returns §30's `export_changed` without exposing a path or labeling the entry that now occupies the final path an owner-removable residual. The next request revalidates that entry from the beginning; every other rule 6 failure keeps the residual contract unchanged. No manifest field grants path authority.
