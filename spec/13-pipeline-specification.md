...
9. Invalidated-view reporting: the transaction that supersedes or purges current snapshots and branches also captures each affected assessment view — scope, scope target, snapshot ID — and each affected branch name. The invoking command reports every invalidated view and branch together with its explicit §14.9/§14.10 regeneration command; after owner deletion this report is command output only, never persisted derived state. A bare `recompute` retried after a crash rebuilds Stages 3–5 and reports that no current assessment view exists, pointing at §14.9; it never infers a desired view set from historical or purged rows.
