...
Retention is legal only when the candidate's gap and contradiction structural-key sets both equal the current generation's and every gap in the current generation has `answered = false`. When both conditions hold, the prior current generation is retained with its existing rows, IDs, and prose — a paraphrased candidate with equal keys changes nothing — Stage 4 supersedes nothing, and the run records only privacy-safe telemetry; the discarded candidate persists nowhere else. A direct §14.7 invocation then invalidates no upper layer or managed export, while inside the §14.12 flow the downstream stages still follow §13.13. `answered` and `answer_log_id` are service lifecycle state, not detector content, and never enter the comparison. A current generation containing any gap with `answered = true` is always replaced even when the key sets are equal: the detector derived its candidate from inputs that include the stored answer evidence, so a re-emitted gap is genuinely still open (§14.7); each replacement gap starts with `answered = false` and `answer_log_id = None`, and no question-to-answer link is re-created. Otherwise the run replaces both complete sets together, and the prior current generation becomes superseded in the same transaction; inputs or output references to superseded rows are invalid. One Stage 4 run creates one §13.4 `processing_runs` row whether it retains or replaces the generation.
