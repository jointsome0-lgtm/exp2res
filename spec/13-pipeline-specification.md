...
14. When a lineage's §15.2 call supplied a non-empty `displaced_support_items` array and no fact of the committed replacement generation selects any of its descriptors, the completed run's §14.14 result carries one deterministic service-owned warning of type `displaced_support_unselected` for that lineage, naming the lineage root and every unselected descriptor ID in its message. The warning is presentation, not a gate: it persists nowhere, blocks nothing, and recurs on every such run, because legitimate omission exists — displaced support can be off-topic for every replacement fact — while §15.2's required-selection rule means a silent omission would otherwise leave §9.4's descriptor-borne ceilings unreachable without any owner-visible trace.
