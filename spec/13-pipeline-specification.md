...
13. The claim's `dimension` names what the claim characterizes under §15.4's categorization; a mis-categorizing dimension — an orientation dimension on a capability assertion, or the reverse — is `rejected`, because §17 keys section placement on this field and qualification cannot fix a routing value.
