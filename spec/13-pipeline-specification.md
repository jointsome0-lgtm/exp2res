...
9. **Locked-database binding.** Every managed-set removal is bound to the identity of the database held under the §8.1 writer lock: that identity is established when the lock is acquired, and a removal proceeds only while the workspace pathname still resolves to it.
    ...
    - "Every" is exhaustive: selective invalidation, rule 5's preamble, and the total sweeps a §13.13 rule 5 privacy deletion and §14.16's purge run over the managed parents and the migration backup store. The preamble refuses whole rather than by operation, because promoting a surviving rollback mutates a foreign tree exactly as its removals do. A total sweep reports its root, because the names it would otherwise enumerate are the replacement's and say nothing about what was stranded.
