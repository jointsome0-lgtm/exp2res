...
|-------|-----------------------|--------------------|
| 0 — Skeleton | Runtime skeleton (§8, §14.1) + Stage 1, manual capture only | Local database can be created; daily and retrospective logs can be added with linked `manual_claim` evidence items and inspected. |
| 1 — Fact Extraction | Stage 3 (evidence items are persisted by Stage 1) | Raw logs and linked evidence items become atomic facts with source_log_ids; no fact can exist without a source. |
...
| 4 — Resume Export | Stages 8 and 10–12 | System generates a job-targeted Markdown resume with evidence map and verification report; unsupported bullets are blocked. |
