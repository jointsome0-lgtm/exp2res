...
23. Assessment verification performs one semantic pass per current claim, and bullet verification performs one whole-pack semantic pass per branch:
...
36. Assessment subject selection is deterministic and service-owned:
    - `global`, V1's only assessment view, supplies every current fact as subject.
    ...
    - An empty subject fails before the provider call.
...
44. Every §14 command obeys one deterministic §14.14 runtime contract:
    ...
    - The operable inspection surface is exactly `db status`, `logs list`, `facts list/show`, `gaps list`, `contradictions list/show`, `assess list/show`, and `runs list/show`; evidence-item, branch/bullet, job-description, deeper historical-generation, and parsed-requirement-dump inspection remains explicitly deferred.
...
47. Job-description deletion and whole-workspace purge are complete, fail-closed privacy lifecycles:
...
48. Every §19-backed import accepts only its own source's closed unwrapped record and keys idempotency by the exact (source system, source identity) pair plus the importer-computed SHA-256 hash of the validated record's §11 canonical bytes:
    - A first import creates one atomic RawLog/EvidenceItem pair and records the named identity/hash metadata, exact replay is a counted no-op, the same identity with a different hash is rejected without mutating retained evidence, and a record under a different identity creates an independent pair regardless of content.
    - A wrapping envelope, a shared version or export-time field, a record-supplied content hash, a record shaped for another source, and every other undeclared field fail at acquisition without selecting a §12.14 migration.
    - The §19.3 GitHub specialization derives its identity as exactly `<repo>@<commit_sha>`, requires `commit_sha` to contain exactly 40 lowercase hexadecimal characters, and rejects an abbreviated, overlong, uppercase, or non-hexadecimal SHA before duplicate classification.
    ...
    - Omitted §10 `OwnerAttribution` materializes as `unknown` before canonical serialization and hash computation, and only `owner` creates `commit_or_pr` evidence while `not_owner` and explicit or default `unknown` create `artifact_reference` evidence.
    - An exact GitHub replay is a counted no-op, while materially changed content under the same identity and a different, correctly recomputed hash is rejected without rewriting existing raw or evidence rows.
    - Every multi-record payload stays within §11's total-object bound and is processed record by record in file order under one §8.1 writer lock, each record persisting in its own transaction: retained and intra-payload exact duplicates remain counted no-ops, a rejected record withdraws no accepted one, and rerunning the same file after interruption or a lost result converges without a cursor or background continuation.
    - Every completed §14.14 import result carries complete, untruncated `accepted`, `duplicate`, and `rejected` counts and input-ordered `{record_number, source_record_id, raw_log_id}` lists that partition all established records and report a created ID only for a committed acceptance.
...
51. Stage 12 assessment and verified-bullet-pack exports satisfy one closed deterministic contract:
    - Repeated export of the same coherent snapshot or branch reproduces golden-file-identical fixed-member bytes and identical manifest member hashes for `report.md`, `self_claims.json`, assessment `evidence_map.json`, `bullet_pack.md`, bullet-pack `evidence_map.json`, and `verification_report.json`.
    - Every JSON companion validates as the exact extra-forbid version-3 §13.12 document, and any missing, extra, mistyped, unsupported-version, duplicate-ID, unresolved-reference, inexact verification row, or incomplete or unused evidence-link closure fails before publication.
    ...
    - §13.10 keeps the first exact duplicate by its canonical order and drops later byte-equal candidates, while semantic near-duplicate detection remains post-V1 and suppression uses no second LLM coherence pass; the single §15.6 batch is the pack's only generation context.
...
56. Every non-prompt owner record entering through `log today --file`, `log retro --file`, `correction add --file`, or `gaps answer --file` is an owner capture that needs no affirmation flag, and no capture command confirms:
...
58. The §17 mirror publishes as two members of one assessment set:
    ...
    - Repeated export of unchanged state reproduces golden-identical `report.html` bytes pinned by its `manifest.json` member hash under `manifest_version = 6`, with no wall-clock value, nonce, workspace path, or host name in the file.
...
59. `view serve` serves §30's mirror and gap-question views over loopback and nothing else:
    ...
    - Every request resolves one explicit URL selector — an assessment-view identity or one snapshot ID, never a default, an omitted-selector fallback, or a latest-of-several rule — then performs §13.14's complete current-output revalidation and the §16.11 gate before any content is emitted.
