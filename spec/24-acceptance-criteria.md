...
26. Stage 4 is LLM-backed through §15.8, replaces the complete current gap/contradiction candidate set, restricts targets to supplied Stage 1 evidence and current Stage 3 facts, exposes no verdict or resolution state, uses §15.1 only for invalid structured output or references, and rejects generated-voice violations atomically without another LLM call.
