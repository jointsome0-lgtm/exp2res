...
58. The §17 mirror publishes as two members of one assessment set:
    ...
    - Repeated export of unchanged state reproduces golden-identical `report.html` bytes pinned by its `manifest.json` member hash under `manifest_version = 4`, with no wall-clock value, nonce, workspace path, or host name in the file.
