...
47. Job-description deletion and whole-workspace purge are complete, fail-closed privacy lifecycles at their own scopes: each attempts every dependent managed path, commits its database deletion regardless, and reports every residual instead of success; deletion retains everything that does not depend on the selected vacancy while leaving none of that vacancy's own source text, derived prose, or content-derived telemetry hash, and purge leaves none of any in the workspace (§13.13 rule 10, §14.15, §14.16).
