...
26. Stage 4 is LLM-backed through §15.8, replaces the complete current gap/contradiction candidate set, and restricts targets to supplied Stage 1 evidence and current Stage 3 facts; the persisted §11 model, §12 rule 10 validator, and §15.8 producer expose exactly the same closed `DetectionRefType` target domain; Stage 4 exposes no verdict or resolution state, uses §15.1 only for invalid structured output or references, and rejects generated-voice violations atomically without another LLM call.
...
29. Project snapshots store the exact scope target without changing the one-current-snapshot-per-`AssessmentScope` replacement key; snapshot unknowns are the complete typed current unanswered `GapQuestion` set with answered rows excluded, and §17 renders those unknowns plus every claim's counterevidence without creating an independent prose input, altering §16.11 aggregation, or bypassing Stage 10 gates.
