...
48. Every §19-backed import accepts only its own source's closed unwrapped record and keys idempotency by the exact (source system, source identity) pair plus the importer-computed SHA-256 hash of the validated record's §11 canonical bytes:
    - A first import creates one atomic RawLog/EvidenceItem pair and records the named identity/hash metadata, exact replay is a counted no-op, the same identity with a different hash is rejected without mutating retained evidence, and the same hash under a different identity creates an independent pair.
