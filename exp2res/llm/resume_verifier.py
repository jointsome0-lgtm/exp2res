...
RESUME_VERIFIER_INSTRUCTIONS = (
    ...
    "a level into it. That ceiling comes from what the evidence says about "
    "ownership and from nothing else: a claim's general confidence does not "
    "change the supported ownership rank, so a high-confidence fact whose "
    "ownership_level is unknown still supports only unknown, and the strength "
    "of a source never lifts contributed to led or owned. Metrics must occur in the supplied source "
    ...
    "precision. A bullet that carries no temporal expression at all makes no "
    "precision claim and has nothing to compare: date-free prose is licensed no "
    "matter how precisely its linked evidence happens to be dated, and it is "
    "never a finding that the evidence is more specific than the bullet. The "
    "rules below apply only once the bullet itself places or dates the work. "
    "Compare always by the UTC instant rather than a displayed local date or "
    ...
    "by role noun, or by name. The bullet is judged by the other half of the same "
    "rule: an external reader consumes it, so it stays subject-free, and a bullet "
    "that refers to the owner at all — in the first or third person, by role noun, "
    "by name, or in the second person you would correctly use in your own reason "
    "— violates §16.14 and is rejected. A third-person noun "
    ...
    "identifiers in the source's own script and spelling, compared under NFC: "
    "a composed and a decomposed spelling of the same name are the same token "
    "and neither is a finding, since byte-exactness is the stricter standard "
    "reserved for quoted source-voice excerpts — "
