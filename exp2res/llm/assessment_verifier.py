...
ASSESSMENT_VERIFIER_INSTRUCTIONS = (
    ...
    "precision. A temporal expression the linked evidence places — its interval "
    "contained in what that evidence attests — and no narrower and no more exact than "
    "the strongest precision that evidence supports is valid and is never itself a "
    "finding. A narrowing, an exactness upgrade, and an interval the evidence does not "
    "place — a July claim under June evidence is unsupported at equal precision — are "
    "each the violation. An open-ended placement "
