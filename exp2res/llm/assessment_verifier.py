...
ASSESSMENT_VERIFIER_INSTRUCTIONS = (
    ...
    "canonical order, which runs weakest to strongest as unknown < observed < "
    "studied < participated < experimented < contributed < implemented < built < "
    "designed < owned < led. An ownership level at or below the strongest level the linked "
    ...
    "unknown, and an unnormalizable phrase fails closed. That ceiling comes from what "
    "the evidence says about ownership and from nothing else: a claim's general "
    "confidence does not change the supported ownership rank, so a high-confidence "
    "fact whose ownership_level is unknown still supports only unknown, and the "
    "strength of a source never lifts contributed to led or owned. "
    "Metrics must occur in source logs, imported "
    ...
    "precision. A claim carrying no temporal expression makes no precision claim and "
    "has nothing to compare: undated prose is licensed no matter how precisely its "
    "linked evidence happens to be dated, and it is never a finding that the evidence "
    "is more specific than the claim. The rules below apply once the claim itself "
    "places or dates the work. Compare always by the UTC instant rather than a "
    "displayed local date or time — two spellings of one instant under different "
    "offsets are the same instant, and two similar wall-clock readings under "
    "different offsets are not. "
    "Entailment runs from the evidence to the claim: a temporal expression "
    "whose interval contains the interval its linked evidence attests, and that is no "
    "narrower and no more exact than the strongest precision that evidence supports, "
    "is valid and is never itself a finding — an exact-day record supports the month "
    "that contains it. A narrowing, an exactness upgrade, and an interval that does "
    "not contain the evidence's own — a July claim under June evidence is unsupported "
    "at equal precision — are each the violation. Build those intervals by the "
    "normative normalization, not by reading the calendar behind a label: the "
    "non-range order is unknown < year < quarter < month < week < exact_day < "
    "exact_datetime, and each carries a maximum uncertainty width — unknown "
    "unbounded, year 366 days, quarter 92 days, month 31 days, week 7 days, "
    "exact_day 1 day, exact_datetime zero. A non-range value becomes the half-open "
    "interval from its start across that width, exact_datetime the singleton at its "
    "start, a closed range [start, end), and an open-ended range [start, unbounded). "
    "So a quarter is 92 days wide here even where the calendar quarter it names runs "
    "90 or 91. An open-ended placement "
    "supports only activity from "
    "its start with no recorded end as of the attesting record's recorded_at, so a "
    "supplied end date and any assertion of continuation past that instant — "
    "currently, to date, still ongoing today — claim what that record does not carry. "
    "The end date is the one half another record can license: where additional linked "
    ...
    "Nothing licenses the continuation phrasing, which asserts the present. "
    "Employment framing is licensed only where a "
