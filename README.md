...
- [`exp2res/`](exp2res/) — the implemented Python package, currently covering workspace management, manual capture, owner deletion and purge, the isolated Codex and Claude runner substrates, Stages 3–7 from fact extraction through verification with the correction/recompute flow, Stages 8 and 10–12 from job-description parsing through the verified bullet-pack export, assessment export, and raw-log/fact/gap/contradiction inspection.
...
- **Local-first and private by default.** The owner's workspace is the only canonical store; nothing is sent anywhere on system initiative. LLM calls happen only inside explicit, user-initiated pipeline runs against an explicitly selected provider; those commands run without a second cost prompt, while `--yes` remains reserved for destructive deletion, purge, and migration consent.
- **Processing privacy is provider choice.** Local-first describes storage, not the model call: whatever a stage sends, the selected provider sees — [spec §29.3](spec/29-security-and-privacy.md) lists the exact classes per contract. There is no per-record "do not send" marker, and none is planned for V1. If some material must not reach a third party, select a provider you trust with everything — this build's default is the Codex CLI adapter on `gpt-5.6-sol` — or wait for the self-hosting route: [spec §15.13](spec/15-llm-contracts.md) requires an `openai-compat` adapter whose `base_url` can point at a locally hosted endpoint, and this build does not register it yet (see below). Material you decline to record is the only material Exp2Res never sends today.
...
The repository contains runnable implementation slices, including local workspace/capture behavior, the pipeline through assessment verification and export, and isolated runner infrastructure. The [ecosystem-wide security policy](https://github.com/jointsome0-lgtm/selfos/blob/main/SECURITY.md) governs vulnerability reporting, while [spec §29](spec/29-security-and-privacy.md) remains the canonical Exp2Res security, privacy, and managed-data boundary.
...
Each adapter also accepts an optional absolute path to its local executable — `codex_binary_path` and `claude_binary_path`. Omit them and the adapter discovers its CLI on `PATH`. Set one when `PATH` resolves to something the isolated runner cannot execute, such as an npm launcher shim whose interpreter is not inside the sandbox:

```toml
codex_binary_path = "/usr/lib/node_modules/@openai/codex/node_modules/@openai/codex-linux-x64/vendor/x86_64-unknown-linux-musl/bin/codex"
```

The value names a local program, never a credential; a missing or non-executable path fails `capability_mismatch` before any provider call.
