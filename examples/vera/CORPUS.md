...
`vera-example-fixtures` **v0.2.0** — the one reusable public evidence
...
The E1 fact-extractor responses are canned provider-body bytes, not golden
business outputs: replay assertions stay at the coarse outcome and provenance
level declared by `derived_rule`. Deliberately not here yet, and planned as
additive layers once their owning consumers exist: later-stage canned §15
responses, golden expected outputs only where §21 fixes service-owned
determinism, and the §14.7 gap-answer scripts.
