...
- §8 Runtime Architecture — Python, Typer, SQLite, Pydantic; CLI-first, with database compatibility and migration owned by §12.14
...
- §12 SQLite Schema — per-entity-table `TEXT PRIMARY KEY` identity, derived typed JSON storage, normalized fact provenance, transactional reference validation, and normative DDL for fact_sources, processing_runs, and append-only schema_meta; fail-closed compatibility plus verified-backup, single-transaction migration
- §13 Pipeline Specification — 10 active stable-number stages with conservative occurred provenance, LLM-backed Stage 4/8 contracts, fact-mediated gap answers, typed assessment content, snapshot-anchored resume generation, Stage 3–5 lifecycle recompute with explicit view regeneration, and owner deletion of managed exports and migration backups
- §14 CLI Specification — sole command-form authority; idempotent non-destructive init, pre-business-I/O compatibility, read-only database status, explicit migration, capture/local-payload import, owner deletion, extract/recompute, generation with explicit selectors and persisted project scope targets, verifier findings, export
...
- §24 Acceptance Criteria — 39 V1 checks including schema compatibility and verified atomic migration plus the per-table primary-key identity and no-reuse contract
